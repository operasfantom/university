...
// Invariant: size >= 0  && for i in [pointerFront, pointerBack) storage[i] != null && this not is null

// Immutable: for i in range(size) storage[next(pointerFront)^(i)] == storage'[next'(pointerFront')^i] && size == size'

public class ArrayQueueModule {

    final static int initialSize = 1;
    private static int size = 0, pointerFront = 0, pointerBack = 0;
    private static Object[] storage = new Object[initialSize];
    ...
    // Post: size = size' + 1 &&
    // for i in range(size') storage[next(pointerFront)^i] == storage'[next'(pointerFront')^i] &&
    // storage[next(pointerBack)] == element
    //---
    ...
    // Pre: Invariant && size > 0
    // Post: size == size' - 1 &&
    // for i in range(1, size) storage[next(pointerFront)^i] == storage'[next'(pointerFront')^i] &&
    // R == element()
    //---
    ...
    // for i in range(size) storage[next(pointerFront)^(i + 1)] == storage'[next'(pointerFront')^i] &&
    ...
    // Pre: Invariant && size > 0
    // Post: Invariant && size == size' - 1
    // for i in range(size - 1) storage[next(pointerFront)^(i)] == storage'[next'(pointerFront')^i]
