...
public class ExpressionTokenizer {
    ...
    <T extends AbstractNumber<T>> List<Token> tokenize(String s, T number) throws CheckedParserException {
        ...
        for (int i = 0; i < s.length(); ) {
            ...
            switch (c) {
                ...
                default: //numbers and letters
                    ...
                    String buf = number.reader(s, i);

                    if (buf.isEmpty()) {
                        i = readIdentifier(s, i, token);
                        result.add(token);
                    } else {
                        result.add(new Token(NUMBER, buf));
                        i += buf.length();
                    }
                    break;
            }
